...
func getSecrets(pod *corev1.Pod) []string {
	...
	for _, v := range pod.Spec.Volumes {
		...
		case v.Secret != nil:
			...
			fmt.Printf("pod %s depends on secret %s", pod.Name, v.Secret.SecretName)
			secretNames = append(secretNames, v.Secret.SecretName)

		case v.CephFS != nil:
			fmt.Printf("pod %s depends on secret %s", pod.Name, v.CephFS.SecretRef.Name)
			secretNames = append(secretNames, v.CephFS.SecretRef.Name)
		case v.Cinder != nil:
			fmt.Printf("pod %s depends on secret %s", pod.Name, v.Cinder.SecretRef.Name)
			secretNames = append(secretNames, v.Cinder.SecretRef.Name)
		case v.RBD != nil:
			fmt.Printf("pod %s depends on secret %s", pod.Name, v.RBD.SecretRef.Name)
			secretNames = append(secretNames, v.RBD.SecretRef.Name)
		default:
			fmt.Printf("Skip other type volumes")
		}
	}
	...
	fmt.Printf("pod %s depends on secrets %s", pod.Name, secretNames)
	return secretNames
}
...
func getConfigmaps(pod *corev1.Pod) []string {
	...
	fmt.Printf("pod %s depends on configMap %s", pod.Name, cmNames)
	return cmNames
}
...
func getPVCs(pod *corev1.Pod) []string {
	...
	fmt.Printf("pod %s depends on pvc %v", pod.Name, cmNames)
	return cmNames
}
